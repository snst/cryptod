...
static bool valid_crypto_msg_req(crypto_msg_header_t *hdr)
{
    return (hdr->magic == CRYPTO_MAGIC_REQ) && (hdr->version == CRYPTO_PROTO_VERSION);
}

static bool valid_crypto_msg_res(crypto_msg_header_t *hdr)
{
    return (hdr->magic == CRYPTO_MAGIC_RES) && (hdr->version == CRYPTO_PROTO_VERSION);
}

static bool valid_crypto_msg_ok(crypto_msg_header_t *hdr)
{
    return (crypto_code_t)(hdr->status) == crypto_code_t::OK;
}

static const char *dump_crypto_msg(crypto_msg_header_t *hdr)
{
    static char buf[256];
